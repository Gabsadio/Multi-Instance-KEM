...
if __name__ == "__main__":
    ...
    for n, m, w in mceliece_parameters:
        print(f"mceliece{n}{w} ...")
        k = n - m * w

        # Single-Instance:
        print("Single-Instance:\nDS: ", end="")
        timeds, memds, (mmds, pds, lds, rds) = dsDOOM(n, k - 1, w, 1)
        print(timeds)
        print("MMT: ", end="")
        timemmt, memmmt, (mmmmt, pmmt, lmmt, bmmt, p_, rmmt) = mmtDOOM(n, k - 1, w, 1)
        print(timemmt)
        ...
        with Pool(processes=min(64, cpu_count())) as pool:
            input = [(n, k - 1, w, 1 << i) for i in instanceRange[1:]]
        ...
        with open(f"Out/McElieceDOOM/{n}{w}.txt", "w") as file:
            file.write("DS-DOOM\n")
            file.write("-------\n")
            file.write(
                "log2 # Instances      :\t"
        ...
        y = 143 * (n == 3488) + 207 * (n == 4608) + 272 * (n in [6688, 6960, 8192])
        plt.axhline(y, color="tab:gray", ls="--")
        plt.legend()
        plt.xlabel("$\\log_2(M)$")
        plt.ylabel("$\\log_2$ runtime")
        plt.savefig(f"Figures/McElieceDOOM/{n}{w}/Runtimes")
        ...
        plt.savefig(f"Figures/McElieceDOOM/{n}{w}/Speedups")
        ...
        plt.savefig(f"Figures/McElieceDOOM/{n}{w}/Memory")
