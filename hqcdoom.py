...
if __name__ == "__main__":
    ...
    for j, (n, w, ell, lambd) in enumerate(hqc_parameters):
        print(f"HQC-{2*j+1} ...")
        # Single-Instance:
        print("Single-Instance:\nDS: ", end="")
        timeds, memds, (mmds, pds, lds, rds) = dsDOOM(
            3 * n, n + ell + lambd - 3, 3 * w, 1
        )
        print(timeds)
        print("MMT: ", end="")
        ...
        with Pool(processes=min(64, cpu_count())) as pool:
            input = [
                (3 * n, n + ell + lambd - 3, 3 * w, 1 << i) for i in instanceRange[1:]
            ]
