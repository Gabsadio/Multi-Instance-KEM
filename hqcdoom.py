...
if __name__ == "__main__":
    plt.style.use("bmh")
    hqc_parameters = [(17669, 75), (35851, 114), (57637, 149)]
    for j, (n, w) in enumerate(hqc_parameters):
        print(f"HQC-{2*j+1} ...")

        # Single-instance:
        print("Single-instance:\nDS: ", end="")
        timeds, memds, (mmds, pds, lds, p_ds, kl_) = dsDOOM(2 * n, n - 2, 2 * w, n)
        print(timeds, "\nMMT: ", end="")
        timemmt, memmmt, (mmmmt, pmmt, lmmt, bmmt, rkl_, p_mmt, rp_) = mmtDOOM(
            2 * n, n - 2, 2 * w, n
        )
        print(timemmt)
        ...
        MMTparameters_rkl_ = dict((i, rkl_) for i in instanceRange)
        ...
        for i in instanceRange[1:]:
            ...
            if MMTruntimes[i] > MMTruntimes[i - 1]:
                ...
                MMTparameters_rkl_[i] = MMTparameters_rkl_[i - 1]
        ...
        with open(f"Out/HQCDOOM/HQC-{2*j+1}.txt", "w") as file:
            file.write("DS-DOOM\n")
            file.write("-------\n")
            file.write(
                "log2 # Instances      :\t"
            ...
            file.write(
                "rkl                   :\t"
                + "\t".join(f"{entry:20d}" for entry in MMTparameters_rkl_.values())
                + "\n"
            )
            file.write(
                "p_                    :\t"
